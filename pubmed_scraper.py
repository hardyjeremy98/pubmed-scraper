...
class ContentFetcher:
    """Handles fetching full text content and abstracts."""
    ...
    def get_full_article_content(self, article: ArticleMetadata) -> ArticleMetadata:
        ...
        if not article.content and article.pmcid:
            content = self._fetch_fulltext_from_pmc(article.pmcid)
            if content:
                print("Content found")
    ...
    def _fetch_fulltext_from_pmc(self, pmcid: str) -> Optional[str]:
        """Retrieve fulltext from PMC if available."""
        if not pmcid:
            return None
        ...
        try:
            ...
            r = self.http_session.get(oa_url)

            print(r.status_code, r.url)

            if r.status_code == 200:
                print("Full text found in Europe PMC")
            ...
            print(f"Error fetching fulltext for {pmcid}: {e}")
...
class PDFDownloader:
    ...
    def download_pdf_from_pmcid(
        ...
        pmid_dir = os.path.join(self.base_dir, pmid)
...
class ImageDownloader:
    ...
    def get_pmc_figures(self, pmc_url: str) -> List[Figure]:
        """Extract figures from PMC article."""
        try:
...
class ArticleManager:
    ...
    def save_article_to_json(
        ...
        # Create PMID-specific directory
        pmid_dir = os.path.join(base_dir, article.pmid)
        os.makedirs(pmid_dir, exist_ok=True)
    ...
    def save_figures_to_json(
            ...
            figures_data.append(
                {
                    ...
                    "local_filename": f"figure_{i+1}.{file_ext}",
                }
            )
        ...
        print(f"Saved figures metadata to {filepath}")
        return filepath
...
class PubMedClient:
    ...
    def get_article_figures(self, pmid: str) -> List[Figure]:
        """Get figures for an article by PMID. Returns figures if PMCID is available."""
        article = self.get_article_metadata(pmid)
        if article.pmcid:
            pmc_url = f"https://pmc.ncbi.nlm.nih.gov/articles/{article.pmcid}"
            return self.get_pmc_figures(pmc_url)
        else:
